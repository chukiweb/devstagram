...
@section('contenido')
                    ...
                    @if($user->id !== auth()->user()->id)
                        ...
                        @if(!$user->siguiendo(auth()->user()))
                            ...
                            <form action="{{ route('users.follow', $user )}}" method="POST">
                                @csrf
                                <input class="bg-blue-600 text-white uppercase rounded-lg px-3 py-1 text-xs font-blod cursor-pointer" type="submit" value="Seguir">
                            </form>
                        @else   
                            <form action="{{ route('users.unfollow', $user)}}" method="POST">
                                @csrf
                                @method('DELETE')
                                <input class="bg-red-600 text-white uppercase rounded-lg px-3 py-1 text-xs font-blod cursor-pointer" type="submit" value="Dejar de Seguir">
                            </form>
                        @endif
    ...
    {{-- Section de mostrar las publicaciones --}}
